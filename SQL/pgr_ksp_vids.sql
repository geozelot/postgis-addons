-- DROP FUNCTION IF EXISTS pgr_ksp(TEXT, BIGINT[], INTEGER, BOOLEAN, BOOLEAN);
  ...
  BEGIN
    ...
    RETURN QUERY
      ...
      SELECT  (ROW_NUMBER() OVER(ORDER BY ksp.path_id, ksp.itr, ksp.path_seq))::INTEGER AS seq,
              ...
              ksp.COST AS "cost",
              CASE
              	WHEN ksp.edge = -1 THEN
                  (SUM(ksp.cost) OVER(PARTITION BY ksp.path_id ORDER BY ksp.itr, ksp.path_seq))::DOUBLE PRECISION
              	ELSE
                  (SUM(ksp.cost) OVER(PARTITION BY ksp.path_id ORDER BY ksp.itr, ksp.path_seq) - 1)::DOUBLE PRECISION
              END AS agg_cost
      FROM    (
        SELECT  (_pgr_ksp(
                  edges_sql::TEXT,
